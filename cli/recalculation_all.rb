...
class MyCLIProcess < BlackStack::MyLocalProcess
  ...
  def process(argv)
    ...
    # process 
    begin
			['ssm', 'drx'].each { |service|
				self.logger.logs "Service #{service}... "
				BlackStack::Client.where(:delete_time=>nil).all { |c|
					self.logger.logs "#{c.id.to_guid}.#{c.name}... "
					c.recalculate(service)
					self.logger.done
					# libero recursos
					GC.start
					DB.disconnect
